...
`zelos.bastion`) implement it and link back here.
...
| `<bed-or-cluster>` | the deployment instance (a foundry **bed** or a kube **cluster** / environment) | `alpha`, `beta`, `prod`, `staging` |
| `<product>` | the Zelos product the deployment belongs to | `foundry`, `zelosai`, `bastion` |
| `<domain>` | the registered DNS root | `zelosai.cloud` (ZelosAI's own infra); customers use their own |

**Worked example (alpha foundry bed):** `harbor.alpha.foundry.zelosai.cloud`.

The four labels read left-to-right from most-specific to least: *which service*, on *which
instance*, of *which product*, under *which org/customer domain*. One glance localises any
hostname in the suite — useful when troubleshooting several clusters/products at once.
...
| Product | What it is | Typical instance label | Example host |
|---|---|---|---|
| `foundry` | the CI / test-bed platform (per-bed) | bed name (`alpha`, `beta`) | `argocd.alpha.foundry.zelosai.cloud` |
| `zelosai` | the Zelos AI platform (operator + gateway/mcp/backplane) | cluster/env (`prod`) | `gateway.prod.zelosai.<domain>` |
| `bastion` | the standalone OOB recovery gateway (single-service) | env (`prod`) | `prod.bastion.zelosai.cloud` |

New products slot in by adding a `<product>` label; nothing else about the standard changes.
...
|---|---|---|---|
| **public** | `<bed>.<product>.<domain>` → `*.<bed>.<product>.<domain>` | WAN A-record (DDNS / router) via public DNS | `zelos-public-tls` |
| **LAN** | `<bed>.<product>.local` → `*.<bed>.<product>.local` | bed VIP via in-cluster dnsmasq | `zelos-local-tls` |
...
The public names also resolve **on the LAN** (the bed's dnsmasq answers `*.<bed>.<product>.<domain>`
...
cert-manager issues **two** wildcard certificates per instance:

- **`zelos-public-tls`** = `*.<bed>.<product>.<domain>` (+ apex) — issued by **ACME**
...
- **`zelos-local-tls`** = `*.<bed>.<product>.local` (+ apex) — **always** the internal
...
`<bed-or-cluster>` config may set an optional **public port** when the standard `:443` is taken
by another tenant on the same WAN IP. The alpha bed runs publicly on **`:9443`** (its
...
Auth rides on the bed **apex** host so the two identity providers — **Dex** and **Rancher** —
are drop-in interchangeable (mutually exclusive, chosen per instance). Pick **Rancher** when you
...
|---|---|---|
| **OIDC issuer** | `https://<bed-domain><:port>/oidc` | Rancher-native; **Dex reconfigured from `/dex` → `/oidc`** |
| **GitHub OAuth callback** | `https://<bed-domain><:port>/verify-auth` | Rancher-native; Dex's connector redirectURI set to it + an apex route rewrites `/verify-auth` → Dex's `/oidc/callback` |
| **SSO gate** | `auth.<bed-domain>` | oauth2-proxy; cookie scoped to `.<bed-domain>` for cross-subdomain SSO |
| **Apps** | `<svc>.<bed-domain>` | each gated app behind the SSO gate (except registry + webhook hosts) |

**One GitHub OAuth App** per instance, with one Homepage + one callback (`…/verify-auth`),
...
| Fact | Dex | Rancher |
|---|---|---|
| `oidc_issuer_url` | `<bed-domain>/oidc` | `<bed-domain>/oidc` |
...
- **`zelos.kubernetes` / `zelos.foundry` beds** set a `cluster:` identity dict —
  `root_domain`, `product`, `bed` (defaults to the bed name), optional `public_port`. When
  `cluster.root_domain` is set the platform derives every hostname, the dual certs, the SSO
  gate, and the unified OIDC issuer/callback from it (`[always]` facts in `platform.yml` /
  `deploy.yml`: `bed_domain`, `bed_local_domain`, `public_port_suffix`, `oidc_*`). **Unset ⇒
  legacy single-host path-based routing**, byte-identical to pre-standard behaviour.
...
```
# foundry — alpha bed, product foundry, root zelosai.cloud, public port 9443
...
- **Collision-free across instances** — many beds/clusters/products coexist without name clashes,
